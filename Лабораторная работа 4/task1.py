...
class AUTO:
    def __init__(self, Mark: str, Serial: str, numer: str):
        ...
        if not isinstance(Mark, str):
            raise TypeError("Напишите как текст")
        self.Mark = Mark
    ...
    def __repr__(self) -> str:
        """
                ...
                >>> CAR.__repr__()
                """
         #переопределила __rerp__ из задания чтобы его можно было наследовать
        a = self.__dict__
...
class AUTO_with_probeg(AUTO):
    def __init__(self, Mark: str, Serial: str, numer: str,probeg: int, cost : int):
        ...
        if not isinstance(cost, int):
            raise TypeError("Цена это целое число")
        self.cost = cost
    ...
    def __str__(self):
        """
            ...
            Примеры:
                ...
                >>> CAR.__str__()
        """
        return f"Марка машины {self.Mark}, Серийный Номер {self.Serial} , Номер авто {self.Numer}, Количество пройденых киллометров {self.probeg}"
